...
# Path to the instance file (same as CLI uses)
import os
...
def _get_instance_info():
    """Read instance_id and project from the instance file."""
...
async def _log_activity(project: str, action: str, summary: str):
    """Log an activity to session_log. Best-effort, never raises."""
    try:
        instance_id, inst_project = _get_instance_info()
        if not instance_id:
            return
        log_project = project or inst_project or ""
        await execute(
            """
            ...
            VALUES ($1, $2, $3, $4)
            """,
            instance_id, log_project, action, summary,
        )
    except Exception:
...
def register_memory_tools(mcp: FastMCP):
    ...
    async def memory_store(
        ...
        try:
            ...
            await _log_activity(
                project or "", "task_completed",
                f"Stored {scope} {memory_type} memory #{mem_id}: {title}"
            )
    ...
    async def memory_checkpoint(
        ...
        for i, mem in enumerate(memories):
            ...
            try:
                ...
                await _log_activity(
                    project or "", "task_completed",
                    f"Checkpoint: stored {scope} {memory_type} memory #{row['id']}: {title}"
                )
